...
- **Money:** `purchase_price`, `earnest_money`, `earnest_money_days` (the contract-stated EM delivery window in days after acceptance; its counting basis rides `deadline_day_basis_json` under `earnest_money_days`; `None` = contract silent → the requirement library's default applies).
...
1. **OCR (`ocr` stage).** Every PDF/image is read with Amazon Textract (`services/textract_service.py`), concurrency-limited. The mode is settings-driven: with `TEXTRACT_OCR_ONLY_MODE=false` the pipeline uses document analysis with FORMS + SIGNATURES so Textract detects checkboxes as SELECTION_ELEMENT blocks. Because Textract never includes those blocks in a line's own text, each detected checkbox is re-inserted **inline** into the page text at its visual position as `[X]` (checked) / `[ ]` (unchecked) (`annotate_lines_with_selection_markers`, matched to its line by bounding-box overlap and positioned by word x-coordinates), so a clause like "The parties agree that ☐ Seller ☒ Buyer will select a title insurance company" reaches the model as `[ ] Seller [X] Buyer will select …` with its clause context intact — critical for "who orders title", financing type, owner-occupancy, and other checked-box decisions, where a bare form key-value pair like `Buyer = [X]` is ambiguous on a contract with many Buyer/Seller boxes. The persisted OCR line geometry carries the same inline markers so a citation that quotes them still locates its source line. All page text is concatenated into one delimited packet (`build_packet_ocr_text`), each document tagged with id, filename, mime type, and page count.
...
Signature status is judged on the **controlling chain**, not the base document's acceptance boxes: a countered PA signed by all parties plus a fully-signed counter offer is fully executed. Both prompt passes carry the chain rule, and a deterministic backstop (`apply_signature_chain_backstop`) corrects a packet-level `all_parties_signed=false` that the model's own per-document inventory contradicts (every non-rejected/voided document fully signed). The Intake Intelligence verifier also cross-checks every checklist proposal against the upload set (`document_name_match.upload_satisfying`): a proposal whose document is already uploaded is tagged `satisfied_by_document_id` and the wizard auto-accepts and auto-attaches it instead of surfacing a "needs your eyes" ask.

The result carries the extraction, the per-field citations, `needs_human_review` + `review_reasons`, the `double_check` block, and the verified `generated` proposals. The frontend applies the extraction to wizard state (`apply_extraction`), builds evidence citations, stores the double-check and signature status, and (for multi-document packets) calls `POST /api/v1/ai/resolve-documents` to flag controlling-vs-superseded conflicts and missing-document reasons as advisory "verify this" flags. The missing-document detector (`contract_resolution._referenced_documents_missing`) reports each unique missing target once: because the packet parse clones the packet-level `referenced_documents` into every uploaded document's `ai_extracted_data`, a naive pass repeated every gap once per document (the tester's "65 referenced documents not uploaded" wall was 5 real gaps × 13 documents). Two guards prevent it: the resolver's inventory reads each document's own identity (`sequence_number` / `document_effective_at`) from its per-document packet finding rather than the cloned packet-level values (so an uploaded counter offer #2 isn't misfiled under the controlling document's sequence and falsely reported missing), and the detector dedupes by `(family, sequence)` (date-only references by date). The wizard also dedupes reasons defensively before rendering the banner.
...
Every deal-shaping field: price, earnest money, financing type, representation, contract acceptance/closing/possession dates and times, and the contingency terms. Day-count and date fields for contingencies stay in sync (editing "14 days" recomputes the date and vice-versa). The contract-stated EM delivery window (`earnest_money_days`, extracted with its business/calendar basis) rides through to the EMD receipt row so the committed deadline is the contract's, not the library default. Cash deals surface the appraisal election. FSBO can be marked here (only when representing the Buyer) — and the checkbox is cross-checked against the packet's own evidence (`detection.fsbo_reference` + the extracted listing agent): marking FSBO on a contract that names a listing agent shows an inline warning, since a represented seller belongs on the seller contact card, never in the FSBO invite. Server-side, `ForSaleByOwner`/`Client` users are rejected as transaction owners outright. Custom contingencies and a pinned free-text note are supported.
...
The wizard previews the compliance document checklist via `POST /api/v1/transactions/preview-tasks` (which returns tasks, the timeline, and requirements together) and lets the user match uploaded supporting documents to requirement rows, waive rows, or add rows (the checklist-import feature was retired 2026-07-22, decision D3: the endpoint, service and modal were deleted along with the step that hosted them). Accepted AI checklist proposals ride the same `extra_requirements` channel. **Rows never ask for a document already uploaded** (client feedback): `computeRequirementAutoMatches` (`wizardTypes.ts`) auto-attaches uploads to rows one-to-one — first by each upload's own detected type from the packet's per-document findings (alias-mapped to the requirement library vocabulary; contract-family/ambiguous types never match), then by normalized filename ↔ row-name equality (which is what satisfies AI-proposed rows, which carry no `doc_type`), with the legacy single-upload → primary-contract rule as the last fallback. A manual attach/unmatch always wins, and the same pure function drives both the step display and the commit payload (preview equals commit). The list renders in **two groups** (client feedback: future documents must not read as intake asks): "Documents you may have now" (matched rows + intake-plausible doc types — a static `INTAKE_PLAUSIBLE_DOC_TYPES` set, never LLM-inferred) in full row UI, and "Collected as the deal progresses" (unmatched non-intake library rows: CD, deed, title commitment…) collapsed behind a count summary, expandable for edit/waive, auto-opened while searching or when the first group is empty. User-added and AI rows always stay in the main group. Creation happens straight from this step; the full task plan still generates at commit. Added to-dos (AI/user, not contract deadlines) that would land strictly inside the final week before closing are pulled back to closing − 7 days at generation (client rule; `metadata_json.date_floor`).
