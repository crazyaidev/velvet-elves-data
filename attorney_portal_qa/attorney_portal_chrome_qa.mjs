...
async function main() {
  ...
  if (matterUrl && /\/transactions\/[0-9a-f-]{8,}/i.test(matterUrl)) {
    ...
    log('matter-sections', /Overview/i.test(mText) && /Review/i.test(mText) && /Brief/i.test(mText) && /Timeline/i.test(mText) && /People/i.test(mText) && /Activity/i.test(mText) && /Releases/i.test(mText) ? 'PASS' : 'FAIL')
    ...
    await shot(page, 'matter_people')
    const peopleText = await dumpText(page, 'matter_people')
    log('people-pane', /People on this matter|No contacts on file/i.test(peopleText) ? 'PASS' : 'FAIL')
    log('people-copy-tx-page', /from the transaction page/i.test(peopleText) ? 'FAIL' : 'PASS', 'attorney cannot open the agent transaction page')
