...
| **Jake answered** | 18 August 2026 — Jan’s replies sit under each answer |
...
Original recommendations are in each question body. Recorded answers after Jake’s 18 Aug markup:

| \# | Question | Blocking? | Recorded answer |
| :---- | :---- | :---- | :---- |
| 1 | The word Autopilot is used for two different things. What should agents see? | No | Autopilot = send without a tap when the task is authorized and confidence is high enough. Assisted = tap to send. Two different roles. Intake Autopilot renamed **Fast intake**. |
| 2 | After the current named letters, which emails may leave without a tap? | Yes | Wait for Jake’s task spreadsheet. Do not invent letters. Current named list + inspection reminders stay until then. |
| 3 | Buyer / seller closing-information email: attach the Closing Disclosure? | Yes | Confirming IDs **420** (buyer) and **430** (seller). Working rule until ticked: **no CD**. |
| 4 | Should buyers and sellers talk to Aime in this phase? | Yes | This phase: status only (“where is my file?”). Never legal or interpretive. |
...
| 6 | If the deal falls through, what should Aime do? | Yes | Stop automatic mail. Keep the history. Mark failed, not closed. Do not start listing or marketing. No extra party notify. |
| 7 | A later signed amendment changes a date. Who confirms it? | Yes | Confirm on Manual and Assisted. On Autopilot, apply only when the amendment is explicit, complete, and does not conflict. Fuzzy always waits. |
| 8 | On a cash file, who (if anyone) gets “Appraisal Ordered”? | Yes | Keep cash Appraisal Ordered / Completed. Target = **Buyer**, agent in copy. Sell-Cash recipient still open. |
