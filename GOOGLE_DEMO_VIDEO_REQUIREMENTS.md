...
**Prepared by:** Jan
**Date:** August 12, 2026
**Revised:** August 12, 2026 — aligned to production after the day’s frontend and backend deploys (Ask AI spoken-date intents, Contacts refresh, Inbox reading pane for Reply-ready mail, in-app Sent folder).
**Supersedes:** `DEMO_VIDEO_GUIDELINE.txt` and `DEMO_VIDEO_GUIDE_FOR_JAKE.md`
**Record on:** https://app.velvetelves.com (live production, not stage)
...
1. **Record on production**, `app.velvetelves.com`. Stage uses a different Google project; a consent screen filmed there will not match the submission.
2. **Record on the same account Google will log into.** The Trust and Safety credentials must open the workspace the video shows. If the video is Jake’s brokerage account, those are the credentials to send — not a separate test admin.
...
4. **A throwaway Google account** to connect. Its inbox and Sent folder will be on camera. Do not connect a personal mailbox.
5. **A second mailbox** to send the inbound test message from. The body must mention the property address of the deal you will walk in Scene 3.
6. **Ask AI auto-apply is off for the take.** If a proposed task shows **Applied / Ran automatically · your rule**, Approve never appears. Scene 4 requires a visible Approve click. Turn that tenant rule off, then run the prompt below.
7. **The Google scope strings are already live** (`userinfo.email` / `userinfo.profile` in full). This is no longer a recording gate.
...
## Labels that match production

Say these names on camera. The older guides used different words.
...
If a document preview stays on “Preparing viewer…”, select the contract file in the list rather than waiting on a blank pane.
...
- **Documents** and **Compliance**: the checklist of what the file requires, what is in and what is missing. Open the signed contract. If the viewer does not render, click that file in the list.
- **Contacts**: the buyer, seller, agents, lender and title company on the deal
...
On the deal, click **Ask AI**. Ask a fact question first so the reviewer sees deal-grounded answers (address, price, closing date).

Then use a prompt production accepts as a proposed action, including a spoken date:

`Propose adding a task named Confirm title commitment reviewed due August 18, 2026`

A **Proposed action** card appears. Click **Approve**. Open the **Tasks** tab and show the new task on the deal.

If the card instead says **Applied / Ran automatically · your rule**, stop, turn that auto-apply rule off, and re-run the prompt so Approve is on camera.
...
Settings → **Email & E-signature** → **Connect** on Gmail. Direct: `app.velvetelves.com/settings/connections`.
...
Send an email from the second mailbox to the connected account, referencing the property address of the Scene 3 deal.

Then show, in order:

1. **Intelligence → Email → Inbox**, with the message present. Click the row. The right pane opens the original message. If the AI drafted a reply, the same pane shows **Original message** and **Suggested reply** (the row is tagged **Reply ready** and the deal address, not “Not linked”).
2. That it was **matched to the correct transaction** with no manual filing.
3. The **deal's own Email tab** carrying the same message (Inbox on that deal).
4. The transaction's **Activity → Communications** history, showing the message logged as a permanent record (**Matched by party**, Received, Gmail).
5. The AI's reading of it: the category chip (for a closing-date question this is **Question**) and what it extracted.
...
Stay on that Inbox pane, or open the same draft under **Outbox**. Show recipient, subject, body, and the panel showing what it was based on. The on-screen line is **Nothing sends until you approve it**.
...
Then show the message in two places: **Intelligence → Email → Sent**, and the connected Gmail **Sent** folder, sent from the user's own address. The same message is logged back on the deal's Email tab.
...
Workflow → **Closing Calendar** → **Connect calendar** → **Google Calendar**. Direct: `app.velvetelves.com/calendar`.
...
Settings → **Email & E-signature** → **Disconnect** on Gmail. Show the confirmation dialog (it explains that inbound sync and send will stop), then confirm. Show the disconnected state.
...
| `.../auth/gmail.readonly` | Gmail | Scene 6, message matched, logged, read by the AI, original visible in the Inbox pane |
| `.../auth/gmail.send` | Gmail | Scene 8, approved reply in Intelligence → Email → Sent, in Gmail Sent, and on the deal |
...
- **The reviewer logging in to a different workspace than the video.** The credentials in the Trust and Safety thread must match the account on camera.
...
3. Reply to the existing **Trust and Safety email thread** with the link, the reviewer test credentials (the same account as the video), and step-by-step navigation instructions.
