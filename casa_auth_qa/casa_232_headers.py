"""Staging: login does not set a session cookie (CASA 2.3.2). Never prints tokens."""
...
def render(rows: list[tuple[str, str]]) -> None:
    ...
    d.text((48, 32), "2.3.2  Staging login is not a session cookie", font=font(22, True), fill=(24, 24, 24))
    d.text(
        (48, 68),
    ...
    d.text(
        (48, H - 48),
        "api.stage.velvetelves.com  |  CASA_2_3_2_headers  |  28 Aug 2026",
        font=font(12),
        fill=(80, 80, 80),
    )
    out = OUT / "CASA_2_3_2_headers.png"
    im.save(out, "PNG")
    print("wrote", out)
    for label, value in rows:
        print(f"  {label}: {value}")
...
if __name__ == "__main__":
    ...
    rows = [
        ...
        ("HttpOnly session cookie", "none - no session cookie"),
    ]
    render(rows)
    if access:
