...
9. **Communication log (Milestone 4.3):** Immutable, searchable, filterable by date/party/keyword. The standalone `/communications` page (sidebar → Workflow → Communications) groups results by date and supports a "Vendor traffic only" chip plus channel filter (Email today; SMS/Voice flagged as "soon"). Download: one transaction at a time per user; multi-transaction requires admin via the existing `/admin/communication-exports` flow.
