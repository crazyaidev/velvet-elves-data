...
1. Work **by ID**, top to bottom in the index. Remaining items are optional (P3 CloudFront OAC, scanner UI).
...
| P1 | [6.7.1](#671--server-side-secrets) | Secrets Manager console | Owner — **done 31 Aug** | 6 |
| P1 | [6.7.1](#671--server-side-secrets) | CloudTrail secret-access | Owner — **done 31 Aug** | 6 |
...
**Already packed:** write-up + code comments (no secret values). Disconnect is **soft deactivate**. Secrets Manager overview + rotation (Disabled). CloudTrail Event history `GetSecretValue` (no SecretString in crop).
...
- [x] CloudTrail secret-access PNG — `CASA_6_7_1_cloudtrail.png`
